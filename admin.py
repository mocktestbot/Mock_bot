...
def register_admin_handlers(bot):
    ...
    def process_broadcast(message):
        ...
        bot.send_message(message.chat.id, f"✅ **ब्रॉडकास्ट सफलतापुर्वक पूरा हुआ!**\nसफलतापूर्वक भेजा गया: {success} छात्रों को।")
        ...
        try:
            ...
            if info:
                ...
                text = (f"👤 **यूज़र की जानकारी:**\n\n"
                        f"🆔 **ID:** `{info['user_id']}`\n"
                        f"📛 **नाम:** {info.get('first_name', 'N/A')}\n"
                        f"🌐 **Username:** @{info.get('username', 'N/A')}\n"
                        f"📅 **जॉइन डेट:** {date}\n"
                        f"✍️ **कुल टेस्ट दिए:** {info.get('total_tests_attempted', 0)}\n"
                        f"🛡️ **स्टेटस:** {status}")
                bot.send_message(message.chat.id, text, parse_mode="Markdown")
            else: bot.send_message(message.chat.id, "❌ यह यूज़र डेटाबेस में नहीं मिला।")
        except ValueError: bot.send_message(message.chat.id, "❌ कृपया सही नंबर (ID) डालें।")

    def make_image_link(message):
        if message.content_type != 'photo':
            bot.send_message(message.chat.id, "⚠️ कृपया सिर्फ फोटो भेजें।"); return
        load_msg = bot.send_message(message.chat.id, "⏳ फोटो अपलोड हो रही है...")
        try:
            file_info = bot.get_file(message.photo[-1].file_id)
            downloaded_file = bot.download_file(file_info.file_path)
            res = requests.post('https://telegra.ph/upload', files={'file': ('image.jpg', downloaded_file, 'image/jpeg')}, timeout=15)
            if res.status_code == 200 and 'src' in res.json()[0]:
                bot.edit_message_text(f"✅ **इमेज लिंक तैयार है:**\n\n`https://telegra.ph{res.json()[0]['src']}`", message.chat.id, load_msg.message_id)
            else: bot.edit_message_text("❌ अपलोड फेल (Telegraph Server Error).", message.chat.id, load_msg.message_id)
        except Exception as e:
            bot.edit_message_text(f"❌ API जवाब नहीं दे रहा (Timeout/Error)।", message.chat.id, load_msg.message_id)
    ...
    def pub_t(call):
        ...
        if t_data:
            msg = (f"🚀 **नया मॉक टेस्ट लाइव हो गया है!**\n\n📚 **परीक्षा:** {t_data['exam_name']}\n📖 **विषय:** {t_data['subject_name']}\n📝 **टेस्ट:** {t_data['test_name']}\n📊 **कुल प्रश्न:** {len(t_data['questions'])}\n🎯 **कट-ऑफ:** {t_data['cutoff']}\n\n👇 **अभी बॉट में जाएं और अपनी तैयारी जांचें!**")
            try: bot.send_message(config.PUBLIC_CHANNEL_ID, msg)
            except: pass
        bot.edit_message_text("✅ **टेस्ट लाइव कर दिया गया है!**", call.message.chat.id, call.message.message_id)
    ...
    def del_menu(call):
        ...
        if call.data == "del_exm":
            ...
            msg = bot.send_message(call.message.chat.id, "🗑 **Test ID भेजें:**")
    ...
    def ban_user(m):
        try:
            ...
            bot.send_message(m.chat.id, f"✅ बैन स्टेटस बदल दिया गया।")
